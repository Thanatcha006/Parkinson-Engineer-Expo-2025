...
if is_started or st.session_state.consent_accepted:
    ...
    # PROCESS BUTTON
            ...
            if st.button("🔍 ประมวลผลทั้งหมด", type="primary", use_container_width=True):
                ...
                # --- HELPER: ฟังก์ชันดึงค่าความน่าจะเป็น (Probability) ---
                def get_model_probability(model, input_data):
                    # กรณี 1: ถ้าโมเดลมีฟังก์ชัน predict_proba (เช่น Scikit-Learn: RF, KNN, SVM(prob=True))
                    if hasattr(model, "predict_proba"):
                        try:
                            probs = model.predict_proba(input_data)
                            # probs จะออกมาเป็น [[prob_class_0, prob_class_1]]
                            # เราต้องการค่าความน่าจะเป็นของการ "เป็นโรค" (Class 1)
                            return probs[0][1] 
                        except Exception:
                            pass # ถ้า Error ให้ข้ามไปใช้วิธีถัดไป
    
                    # กรณี 2: ถ้าเป็นโมเดล TensorFlow/Keras หรือโมเดลที่ predict ออกมาเป็น float อยู่แล้ว
                    raw_pred = model.predict(input_data)
                    
                    # เช็คว่าเป็น Array ซ้อนหรือไม่
                    if hasattr(raw_pred, "ndim") and raw_pred.ndim > 1:
                        val = raw_pred[0][0]
                    else:
                        val = raw_pred[0]
                    
                    # ถ้าค่าออกมาเป็น 0 หรือ 1 เป๊ะๆ (Integers) แสดงว่าโมเดลไม่รองรับ Probability
                    # เราจะ Return ค่าเดิม แต่ต้องทำใจว่า Confidence จะเป็น 100%
                    return float(val)
                ...
                if spiral_image is not None and spiral_model is not None:
                    try:
                        ...
                        # [FIX] ใช้ฟังก์ชันดึง Probability แทนการ predict ตรงๆ
                        ...
                        if pred > 0.5:
                            ...
                            status_color = "#388E3C"
                            # ถ้าความน่าจะเป็นในการเป็นโรคคือ 0.2 (20%) -> Confidence ว่าปกติคือ 80%
    ...
    """).strip()
                ...
                if wave_image is not None and wave_model is not None:
                    try:
                        ...
                        # [FIX] ใช้ฟังก์ชันดึง Probability
