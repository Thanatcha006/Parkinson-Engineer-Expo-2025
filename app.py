...
# ตั้งค่าเริ่มต้นให้ wave_model เป็น None (เพื่อจำลองว่ายังไม่มีโมเดล)
wave_model = None
...
""").strip()
            ...
            if wave_image is not None:
                # ตรวจสอบว่ามีโมเดล Wave หรือไม่ (ปัจจุบัน set เป็น None)
                if wave_model is not None:
                    try:
                        input_tensor = preprocess(wave_image)
                        pred = wave_model.predict(input_tensor)[0][0] # สมมติว่าโครงสร้างเหมือน Spiral
                        ...
                        # HTML Card for Wave (เหมือน Spiral เป๊ะ)
                        result_html_wave = textwrap.dedent(f"""
<div class="result-card" style="background-color: {card_bg};">
...
""").strip()
                        wave_result_box.markdown(result_html_wave, unsafe_allow_html=True)
                    except Exception as e:
                        wave_result_box.error(f"Error: {e}")
                else:
                    # กรณีไม่มีโมเดล (ปัจจุบัน) ให้แสดงข้อความธรรมดา
