...
import time
import base64
import textwrap # สำคัญ: ใช้สำหรับแก้ปัญหา HTML แสดงเป็นโค้ด
...
<style>
    ...
    /* Result Card Styles */
    .result-card {
        /* background-color: กำหนด Dynamic ใน Python */
        color: white;
        border-radius: 20px;
        padding: 30px;
        margin-top: 30px;
        ...
        if st.button("🔍 ประมวลผลทั้งหมด", type="primary", use_container_width=True):
            if spiral_image is not None and spiral_model is not None:
                try:
                    ...
                    if pred > 0.5:
                        # ⚠️ กรณีมีความเสี่ยง (สีเหลืองทอง #E4C728)
                        card_bg = "#E4C728"
                        status_text = "⚠️ พบรูปแบบที่อาจสัมพันธ์กับอาการสั่นแบบโรคพาร์กินสัน"
                        status_color = "#856404" # น้ำตาลเข้ม
                        confidence = pred * 100
                        desc_text = "ตรวจพบรูปแบบการวาดที่มีความไม่สม่ำเสมอ<br>ซึ่งอาจสัมพันธ์กับความผิดปกติของการควบคุมการเคลื่อนไหว"
                        recommend_list = """
                            <li>ควรปรึกษาแพทย์ผู้เชี่ยวชาญเพื่อรับการตรวจและวินิจฉัยเพิ่มเติม</li>
                            <li>สามารถทำการทดสอบซ้ำในสภาวะที่ผ่อนคลาย</li>
                        """
                    else:
                        # ✅ กรณีปกติ (สีฟ้า #67ACC3)
                        card_bg = "#67ACC3"
                        status_text = "✅ ไม่พบความผิดปกติเด่นชัด (Normal)"
                        status_color = "#388E3C" # เขียว
                    ...
                    # *** แก้ไข: ใช้ textwrap.dedent และเขียน HTML ชิดซ้าย แก้ปัญหาแสดงเป็น Source Code ***
...
<div class="result-card" style="background-color: {card_bg};">
    ...
    <div class="disclaimer-small">
        ⚠️ หมายเหตุ: ผลลัพธ์นี้เป็นการคัดกรองเบื้องต้นเท่านั้น
    </div>
</div>
""").strip()
